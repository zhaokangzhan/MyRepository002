...
<script type="text/javascript">
	...
	function del(ids) {
		//alert("删除语句ids=" + ids);
		$.post("score_delete.action?ids=" + ids, function(data) {
			$.messager.show({
				title : '提示消息',
				msg : data.msg,
				timeout : 4000,
	...
	function edit(id) {
		...
		$('#formUpdate').form('load', "score_edit.action?id=" + id);

	}
	function update() {
		$('#formUpdate').form({
			url : "score_update.action",
			success : function(data) {
				var jsonData = eval("(" + data + ")");
				$.messager.show({
					title : '提示消息',
...
<TR>
				...
				<TR>
					...
					<td>课程名称：</td>
					<td><INPUT class=textbox style="WIDTH: 180px; height: 25px"
						maxLength=50 name="course_name"></td>
				</TR>
...
<TR>
				...
				<TR>
					...
					<td><INPUT class=textbox style="WIDTH: 60px; height: 25px"
						maxLength=50 name="score_active" id="tupactid"><select
