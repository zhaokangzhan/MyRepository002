...
//关闭添加框，reload
	...
	function del(ids) {
		//alert("删除语句ids=" + ids);
		$.post("stu_delete.action?ids=" + ids, function(data) {
			$.messager.show({
				title : '提示消息',
				msg : data.msg,
				timeout : 5000,
	...
	function edit(id) {
		...
		$('#formUpdate').form('load', "stu_edit.action?id=" + id);

	}
	function update() {
		$('#formUpdate').form({
			url : "stu_update.action",
			success : function(data) {
				var jsonData = eval("(" + data + ")");
				$.messager.show({
					title : '提示消息',
...
<body>
			...
			<TABLE cellSpacing=0 cellPadding=5 border=0>
				...
				<TR>
					...
					<td><INPUT class=textbox style="WIDTH: 180px; height: 25px"
						maxLength=50 name="student_id"></td>
				</TR>
				...
				<TR>
					...
					<td>姓名：</td>
					<td><INPUT class=textbox style="WIDTH: 180px; height: 25px"
						maxLength=50 name="student_name"></td>
				</TR>
				<TR>

					<td>身份证号：</td>
					<td><INPUT class=textbox style="WIDTH: 180px; height: 25px"
						maxLength=50 name="student_idcard"></td>
				</TR>
				...
				<TR>
					...
					<td><INPUT class=textbox style="WIDTH: 60px; height: 25px"
						maxLength=50 name="student_active" id="tupactid"><select
	...
	<!-- 编辑更新权限的表单，默认是隐藏的 -->
	<div id="winUpdate" class="easyui-window" title="修改学生信息"
		...
		<form id="formUpdate" method="post">
			...
			<TABLE cellSpacing=0 cellPadding=5 border=0>
				...
				<TR>
					...
					<td>账号：</td>
					<td><INPUT class=textbox style="WIDTH: 180px; height: 25px"
						maxLength=50 name="student_uuid"></td>
				</TR>
				<TR>
...
<TR>
				...
				<TR>
					...
					<td>状态：</td>
					<td><INPUT class=textbox style="WIDTH: 60px; height: 25px"
						maxLength=50 name="student_active" id="upactid"><select
